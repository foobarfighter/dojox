dojo.provide("phonegap._base.hostenv_browser");

dojo.requireIf(phonegap.config.debugConsole, "phonegap.debug.DebugConsole");

dojo.addOnLoad(function() {
	if (phonegap.config.debugConsole){
		phonegap.debug.DebugConsole.createLayout();
	}
...
dojo.declare("phonegap._UIControls", [], {
	_toolBarNode: null,
	_toolBarItems: {},
	...
	_layoutNode: function() {
		...
		var ws = dijit.registry.byClass("phonegap.Layout");
		if (!ws.length){throw new Error("Cannot find phonegap.Layout");}
		this._cachedLayoutNode = ws.toArray()[0].domNode;
		return this._cachedLayoutNode;
	}
});

phonegap.uicontrols = new phonegap._UIControls();